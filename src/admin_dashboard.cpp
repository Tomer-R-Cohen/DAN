...
AdminTotals calculate_admin_totals(const std::vector<AdminProvider>& providers)
{
    ...
    for (const auto& p : providers) {
        ...
        ++totals.online; totals.physical_vram_mib += p.total_vram_mib;
        totals.offered_vram_mib += p.offered_vram_mib;
        if (p.role == "ASSIGNED") { ++totals.assigned; totals.assigned_vram_mib += p.offered_vram_mib; }
        else { ++totals.spare; totals.spare_vram_mib += p.offered_vram_mib; }
    }
    return totals;
}
...
std::string admin_snapshot_json(const AdminSnapshot& s)
{
    ...
    out << "{\"network_status\":\"" << escape(s.network_status) << "\",\"provider_status\":\"" << escape(s.provider_status)
        ...
        << ",\"offline\":" << t.offline << ",\"physical_vram_mib\":" << t.physical_vram_mib
        << ",\"offered_vram_mib\":" << t.offered_vram_mib << ",\"assigned_vram_mib\":" << t.assigned_vram_mib
        << ",\"spare_vram_mib\":" << t.spare_vram_mib << "},\"providers\":[";
    for (std::size_t i = 0; i < s.providers.size(); ++i) {
        ...
        out << "{\"name\":\"" << escape(p.name) << "\",\"id\":\"" << escape(p.id) << "\",\"gpu\":\"" << escape(p.gpu)
            << "\",\"role\":\"" << escape(p.role) << "\",\"state\":\"" << escape(p.state) << "\",\"total_vram_mib\":"
            << p.total_vram_mib << ",\"offered_vram_mib\":" << p.offered_vram_mib << ",\"tokens_participated\":"
...
void render_admin_terminal(const AdminSnapshot& s)
{
    ...
    std::printf("\x1b[1mNETWORK\x1b[0m\nProviders: %zu online, %zu assigned, %zu spare, %zu offline\n"
        "VRAM: %.1f GB physical | %.1f GB offered to DAN | %.1f GB assigned | %.1f GB spare\n"
        "Generated: %zu tokens | Requests: %zu\n\n",
        totals.online, totals.assigned, totals.spare, totals.offline,
        totals.physical_vram_mib/1024.0, totals.offered_vram_mib/1024.0,
        totals.assigned_vram_mib/1024.0, totals.spare_vram_mib/1024.0,
    ...
    for (const auto& p : s.providers) std::printf("%-18.18s %-28.28s %7.1fGB %-10.10s %-12.12s %10zu\n",
        p.name.c_str(), p.gpu.c_str(), p.offered_vram_mib/1024.0,
