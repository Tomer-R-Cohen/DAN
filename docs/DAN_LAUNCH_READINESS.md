...
| Distribution license and notices | Open | The copyright owner must select DAN's terms before public release. Builders include pinned llama.cpp/sidecar notices, the exact pinned Qwen2.5 Apache-2.0 license, the installed CUDA license, and a tested 109-file Go dependency bundle (including reciprocal source). Installed CUDA 13.3 terms list the packaged runtime/cuBLAS families as redistributable; Microsoft's current list permits unmodified `VC\Redist` files for licensed Visual Studio users. Confirm the release builder is licensed; third-party notices do not grant rights to DAN. |
